...
SingleFitResult FitWithRCut(string Filename, double RCut, int Strategy = Strategy_Normal);
void FitWithRCut(string Filename, vector<double> RCuts);
...
int main(int argc, char *argv[])
{
   ...
   FitWithRCut("TTJets_MRTree.root", CutsToTry);
}

SingleFitResult FitWithRCut(string Filename, double RCut, int Strategy)
{
...
void FitWithRCut(string Filename, vector<double> RCuts)
{
   ...
   for(int i = 0; i < (int)RCuts.size(); i++)
      SingleFitResults.push_back(FitWithRCut(Filename, RCuts[i], Strategy_IgnoreGaussian));

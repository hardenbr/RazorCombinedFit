...
ClassImp(RooRazor2DTailBNN_SYS)
//---------------------------------------------------------------------------
RooRazor2DTailBNN_SYS::RooRazor2DTailBNN_SYS(const char *name, const char *title,
...
//---------------------------------------------------------------------------
RooRazor2DTailBNN_SYS::RooRazor2DTailBNN_SYS(const RooRazor2DTailBNN_SYS& other, const char* name) :
...
//---------------------------------------------------------------------------
Double_t RooRazor2DTailBNN_SYS::evaluate() const
{
  ...
  double xval = X;
  double yval = Y;
  // Input vector for the BNN:
  std::vector<double> vXY(2);
  vXY[0] = xval;
  vXY[1] = yval;
  double wBNN = SingleMu_2012_HLT_j1ptg200_100K(vXY);

  return wBNN*mycoeff*exp(-myexp);
}

// //---------------------------------------------------------------------------
Int_t RooRazor2DTailBNN_SYS::getAnalyticalIntegral(RooArgSet& allVars, RooArgSet& analVars, const char* rangeName) const{
...
// //---------------------------------------------------------------------------
Double_t RooRazor2DTailBNN_SYS::analyticalIntegral(Int_t code, const char* rangeName) const{
   ...
   if(code ==1) { // integral on both X and Y
     ...
     cout << "WARNING IN RooRazor2DTailBNN_SYS: integration code is not correct" << endl;

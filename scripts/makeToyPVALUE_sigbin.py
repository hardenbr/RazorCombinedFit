...
def getHistogramsWriteTable(MRbins, Rsqbins,nBtagbins, fileName, dataFileName, Box, outFolder, printPlots, fit3D, btagOpt):
    ...
    #DIPHOTON these three lines are unncessary 
    ...
    for i in range(0,len(MRbins)-1):
        for j in range(0,len(Rsqbins)-1):
            ...
            if fit3D:
                ...
                print "MR>= %f && MR < %f && Rsq >= %f && Rsq < %f && nBtag >= %i" %(MRbins[i], MRbins[i+1], Rsqbins[j], Rsqbins[j+1], nBtagbins[0])                
            ...
            switchToKDE = False #DIPHOTON
#            switchToKDE = decideToUseKDE(minX,maxX,htemp)

            if switchToKDE: print "USING KDE (minX,maxX,mean,rms): ", minX,maxX,htemp.GetMean(),rms
            del htemp
            ...
            if myhisto.GetEntries() != 0:
                ...
                if pval==pvalmin:
                    print "Warning PVAL = PVALMIN: Setting nsigma to +/- 5!" 

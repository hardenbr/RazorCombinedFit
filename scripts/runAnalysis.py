...
def defineParser():
    ...
    parser.add_option('--gamma', dest="doGamma", default=False, action='store_true',
                  help="Run the GammaGamma analysis") ##RAZOR DIPHOTON##
    return parser
...
if __name__ == '__main__':
    ...
    if options.doGamma: analysis = "DIPHOTON"
    ...
    if options.analysis is not None:
        ...
        #    DalglishFit.DalglishAnalysis(options.output, cfg), SingleBoxFit.SingleBoxAnalysis(options.output, cfg, analysis)] #Not Necessary for DIPHOTON analysis
        a = [SingleBoxFit.SingleBoxAnalysis(options.output, cfg, analysis)] #DIPHOTON
        for aa in a:
            if aa.name == options.analysis:
                ...
                print "perform box.final()"
                if (options.toys == 0): aa.final() #DIPHOTON

...
class FullSimCentral(object):
    ...
    @staticmethod
    def getBTagScale(pt):
        """Return the data/MC scale factor for b and cjets from muon+jet and ttbar data with the CVSM tagger"""
    ...
    @staticmethod
    def getBTagScaleLight(pt, eta, version = 0):
        """Return the data/MC scale factor for light jets from muon+jet and ttbar data with the CVSM tagger"""
        #recipe: https://twiki.cern.ch/twiki/bin/viewauth/CMS/BtagPOG#2012_Data_and_MC_Moriond13_presc
        #payload: https://twiki.cern.ch/twiki/pub/CMS/BtagPOG/SFlightFuncs_Moriond2013.C
        if pt > 800: pt = 800
        if pt < 20: pt = 20
        SFlight = 0
        eta = abs(eta)
        if eta >= 0 and eta < 0.8:
            SFlight = ((1.06238+(0.00198635*pt))+(-4.89082e-06*(pt*pt)))+(3.29312e-09*(pt*(pt*pt)))
        if eta >= 0.8 and eta < 1.6:
            SFlight = ((1.08048+(0.00110831*pt))+(-2.96189e-06*(pt*pt)))+(2.16266e-09*(pt*(pt*pt)))
        if eta >= 1.6 and eta < 2.4:
            SFlight = ((1.09145+(0.000687171*pt))+(-2.45054e-06*(pt*pt)))+(1.7844e-09*(pt*(pt*pt)))
        return SFlight


    @staticmethod
    def getBTagScaleWithFlavour(pt, eta, flav):
        #b and c jets get the same scale
        if abs(flav) in [4,5]:
            return FullSimCentral.getBTagScale(pt)
        else:
            return FullSimCentral.getBTagScaleLight(pt, eta)
...
class FullSimError(object):
    ...
    def getBTagScaleError(pt):
        # Modified by SS
        """Return the error on data/MC scale factor from muon+jet and ttbar data with the CVSM tagger"""
        #recipe: https://twiki.cern.ch/twiki/bin/viewauth/CMS/BtagPOG#2012_Data_and_MC_Moriond13_presc
        #payload: https://twiki.cern.ch/twiki/pub/CMS/BtagPOG/SFb-pt_payload_Moriond13.txt
    ...
    @staticmethod
    def getBTagScaleErrorLight(pt, eta):
        """Return the data/MC scale factor for light jets from muon+jet and ttbar data with the CVSM tagger"""
        #recipe: https://twiki.cern.ch/twiki/bin/viewauth/CMS/BtagPOG#2012_Data_and_MC_Moriond13_presc
        #payload: https://twiki.cern.ch/twiki/pub/CMS/BtagPOG/SFlightFuncs_Moriond2013.C
        if pt > 800: pt = 800
        ...
        if eta >= 0 and eta < 0.8:
            SFlight = ((1.06238+(0.00198635*pt))+(-4.89082e-06*(pt*pt)))+(3.29312e-09*(pt*(pt*pt)))
            SFlightmn = ((0.972746+(0.00104424*pt))+(-2.36081e-06*(pt*pt)))+(1.53438e-09*(pt*(pt*pt)))
            SFlightmx = ((1.15201+(0.00292575*pt))+(-7.41497e-06*(pt*pt)))+(5.0512e-09*(pt*(pt*pt)))
        if eta >= 0.8 and eta < 1.6:
            SFlight = ((1.08048+(0.00110831*pt))+(-2.96189e-06*(pt*pt)))+(2.16266e-09*(pt*(pt*pt)))
            SFlightmn = ((0.9836+(0.000649761*pt))+(-1.59773e-06*(pt*pt)))+(1.14324e-09*(pt*(pt*pt)))
            SFlightmx = ((1.17735+(0.00156533*pt))+(-4.32257e-06*(pt*pt)))+(3.18197e-09*(pt*(pt*pt)))
        if eta >= 1.6 and eta < 2.4:
            SFlight = ((1.09145+(0.000687171*pt))+(-2.45054e-06*(pt*pt)))+(1.7844e-09*(pt*(pt*pt)))
            SFlightmn = ((1.00616+(0.000358884*pt))+(-1.23768e-06*(pt*pt)))+(6.86678e-10*(pt*(pt*pt)))
            SFlightmx = ((1.17671+(0.0010147*pt))+(-3.66269e-06*(pt*pt)))+(2.88425e-09*(pt*(pt*pt)))
        SFlightError = max(abs(SFlight - SFlightmn), abs(SFlight - SFlightmx))
        return SFlightError



    @staticmethod
    def getBTagScaleErrorWithFlavour(pt, eta, flav):
       #see https://twiki.cern.ch/twiki/bin/viewauth/CMS/BtagPOG
        if abs(flav) == 5:
            ...
            return FullSimError.getBTagScaleErrorLight(pt, eta)
...
class FastSimCentral(object):
    ...
    @staticmethod
    def getBTagEfficiencyWithFlavour(pt, flav):
        # Read the fastsim efficiencies from an input file
        f = rt.TFile('btageffs_T2tt.root')
        # Get the efficiency histograms according to flavor
        if abs(flav) == 5:
            h = f.Get('h_btageffb')
        elif abs(flav) == 4:
            h = f.Get('h_btageffc')
        ...
        for i in range(0, h.GetNbinsX()+2):
            if i < h.GetNbinsX()+1:
                if (pt >= h.GetBinLowEdge(i) and pt < h.GetBinLowEdge(i+1)):
                    eff = h.GetBinContent(i)
                    break
            else:
                if (pt >= h.GetBinLowEdge(i)):
                    eff = h.GetBinContent(i)
                    break
...
class BTag(object):
    ...
    def getBTagScaleFastSim(self, tag, pt, eta, flav):
    ...
    def getEfficiencyFastSim(self, tag, pt, eta, flav):
        ...
        return FastSimCentral.getBTagEfficiencyWithFlavour(pt, flav), 0

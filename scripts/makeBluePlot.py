...
def Binning(Box, noBtag):
    if Box == "Jet" or Box=="Jet2b" or Box =="Jet1b" or Box == "TauTauJet" or Box == "MultiJet":
        ...
        else: Rsqbins =       [.01, .02, .03, .05, .1, 0.2, 0.30,0.41,0.52,0.64,0.80,1.5]
    else:
        MRbins = map(lambda(x):x*.001,range(400,1000,50)+[1000, 1060, 1135, 1225, 1330, 1450, 1585, 1735,1870]+range(2000,6000,500))
...
#        if noBtag: Rsqbins =  [0.15,0.20,0.25,0.3,0.35,0.40,0.45,0.50]
        if noBtag: Rsqbins =  [.01, .02, .03, .05, .1, .15, .2, .25,.3, .35, .45, .6, 1.5] #DIPHOTON

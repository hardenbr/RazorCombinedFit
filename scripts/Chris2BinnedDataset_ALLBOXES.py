...
def convertTree2Dataset(tree, nbinx, nbiny, outputFile, config, minH, maxH, nToys, write = True):
    ...
    for box in boxes:               
        ...
        # attempt to use the Had Signal Hist for All Boxes!
        # Wed Oct 5 17:30 PST
        ...
        normBoxInSR = histo.Integral()
        print 'Histogram Integral for Box %s is %f'%(box,normBoxInSR)
        print 'Histogram Entries for Box %s is %f'%(box,histo.GetEntries())
        tree.Project("wHisto_%s" %box, "RSQ:MR", 'LEP_W*W*(MR >= %f && MR <= %f && RSQ >= %f && RSQ <= %f && (BOX_NUM == %i))' % (mRmin,mRmax,rsqMin,rsqMax,boxMap['Had']))
        normHadInSR = histo.Integral()
        print 'Histogram Integral for Box %s is %f'%('Had',normHadInSR)
        print 'Histogram Entries for Box %s is %f'%('Had',histo.GetEntries())
        histo.Scale(normBoxInSR/normHadInSR)
        print 'Histogram Integral for Box %s after rescaling is %f'%(box,histo.Integral())
        print 'Histogram Entries for Box %s after rescaling is %f'%(box,histo.GetEntries())
        ...
        normBoxInSR = histo_JESup.Integral()  
        print 'Histogram Integral for Box %s is %f'%(box,normBoxInSR)
        print 'Histogram Entries for Box %s is %f'%(box,histo_JESup.GetEntries())      
        tree.Project("wHisto_JESup_%s" %box, "RSQ_JES_UP:MR_JES_UP", 'LEP_W*W*(MR_JES_UP >= %f && MR_JES_UP <= %f && RSQ_JES_UP >= %f && RSQ_JES_UP <= %f && (BOX_NUM == %i))' % (mRmin,mRmax,rsqMin,rsqMax,boxMap['Had']))
        normHadInSR = histo_JESup.Integral()
        print 'Histogram Integral for Box %s is %f'%('Had',normHadInSR)
        print 'Histogram Entries for Box %s is %f'%('Had',histo_JESup.GetEntries())
        histo_JESup.Scale(normBoxInSR/normHadInSR)
        print 'Histogram Integral for Box %s after rescaling is %f'%(box,histo_JESup.Integral())
        print 'Histogram Entries for Box %s after rescaling is %f'%(box,histo_JESup.GetEntries())
        ...
        normBoxInSR = histo_JESdown.Integral()  
        print 'Histogram Integral for Box %s is %f'%(box,normBoxInSR)
        print 'Histogram Entries for Box %s is %f'%(box,histo_JESdown.GetEntries()) 
        tree.Project("wHisto_JESdown_%s" %box,  "RSQ_JES_DOWN:MR_JES_DOWN", 'LEP_W*W*(MR_JES_DOWN >= %f && MR_JES_DOWN <= %f && RSQ_JES_DOWN >= %f && RSQ_JES_DOWN <= %f && (BOX_NUM == %i))' % (mRmin,mRmax,rsqMin,rsqMax,boxMap['Had']))
        normHadInSR = histo_JESdown.Integral()
        print 'Histogram Integral for Box %s is %f'%('Had',normHadInSR)
        print 'Histogram Entries for Box %s is %f'%('Had',histo_JESdown.GetEntries())
        histo_JESdown.Scale(normBoxInSR/normHadInSR)
        print 'Histogram Integral for Box %s after rescaling is %f'%(box,histo_JESdown.Integral())
        print 'Histogram Entries for Box %s after rescaling is %f'%(box,histo_JESdown.GetEntries())
        ...
        normBoxInSR = histo_xsecup.Integral()  
        print 'Histogram Integral for Box %s is %f'%(box,normBoxInSR)
        print 'Histogram Entries for Box %s is %f'%(box,histo_xsecup.GetEntries()) 
        tree.Project("wHisto_xsecup_%s" %box, "RSQ:MR",  'LEP_W*W_UP*(MR >= %f && MR <= %f && RSQ >= %f && RSQ <= %f && (BOX_NUM == %i))' % (mRmin,mRmax,rsqMin,rsqMax,boxMap['Had']))
        normHadInSR = histo_xsecup.Integral()
        print 'Histogram Integral for Box %s is %f'%('Had',normHadInSR)
        print 'Histogram Entries for Box %s is %f'%('Had',histo_xsecup.GetEntries())
        histo_xsecup.Scale(normBoxInSR/normHadInSR)
        print 'Histogram Integral for Box %s after rescaling is %f'%(box,histo_xsecup.Integral())
        print 'Histogram Entries for Box %s after rescaling is %f'%(box,histo_xsecup.GetEntries())
        ...
        normBoxInSR = histo_xsecdown.Integral()  
        print 'Histogram Integral for Box %s is %f'%(box,normBoxInSR)
        print 'Histogram Entries for Box %s is %f'%(box,histo_xsecdown.GetEntries()) 
        tree.Project("wHisto_xsecdown_%s" %box, "RSQ:MR",  'LEP_W*W_DOWN*(MR >= %f && MR <= %f && RSQ >= %f && RSQ <= %f && (BOX_NUM == %i))' % (mRmin,mRmax,rsqMin,rsqMax,boxMap['Had']))
        normHadInSR = histo_xsecdown.Integral()
        print 'Histogram Integral for Box %s is %f'%('Had',normHadInSR)
        print 'Histogram Entries for Box %s is %f'%('Had',histo_xsecdown.GetEntries())
        histo_xsecdown.Scale(normBoxInSR/normHadInSR)
        print 'Histogram Integral for Box %s after rescaling is %f'%(box,histo_xsecdown.Integral())
        print 'Histogram Entries for Box %s after rescaling is %f'%(box,histo_xsecdown.GetEntries())
